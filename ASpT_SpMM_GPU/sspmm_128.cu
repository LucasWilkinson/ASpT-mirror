...
void ready(int argc, char **argv)
{
	...
	fpo2 = fopen("SpMM_GPU_SP_preprocessing.out", "a");
        FILE *fp;
        int *loc;
        char buf[300];
        int nflag, sflag;
...
void spmv_kernel64_sparse_v2h(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *special, int *special2)
{
	...
	__shared__ FTYPE sout2[SPBSIZE];
...
void spmv_kernel64_dense_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *baddr, int *saddr)
{
	...
	int dummy = mcsr_cnt[base_addr]*BH + i*(mcsr_cnt[base_addr+1] - mcsr_cnt[base_addr]) + stride;
        ...
        if(interm3 < loc2) {
                ...
                r2 += __shfl(buf2, jj,MFACTOR) * sin2[__shfl(buf, jj,MFACTOR)][lane];
        }
...
void process()
{
	...
	//TODO opt-space
	cudaMalloc((void **) &_mcsr_e, sizeof(int)*ne+256*1);
	...
	cudaMemset(_mcsr_e, 0, sizeof(int)*ne+256*1);
...
//printf("%f,", ptot_ms);
	...
	dim3 d_gridsize(num_dense, 1, CEIL(sc, MFACTOR*2));
	dim3 d_blocksize(DBSIZE, 1, 1);
	dim3 s_gridsizeh(special_p, 1, CEIL(sc, MFACTOR*2));
...
for(int ik=0;ik<ITER;ik++) {
	// kernel fun
        spmv_kernel64_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
}
...
for(int ik=0;ik<ITER;ik++) {
	// kernel fun
        spmv_kernel64_sparse_v2<<<s_gridsize, s_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
        spmv_kernel64_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
}	
	cudaEventRecord(event2,0);
	cudaEventSynchronize(event1);
	cudaEventSynchronize(event2);
...
for(int ik=0;ik<ITER;ik++) {
	// kernel fun
        ...
        spmv_kernel64_sparse_v2h<<<s_gridsizeh, s_blocksizeh, 0, stream3>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _special, _special2);
        spmv_kernel64_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
}	
	cudaEventRecord(event2,0);
	cudaEventSynchronize(event1);
