...
int sflag=0;
		while(c == '%')
		{
			...
			c = infile.get();
			if(flag == -1) {
				if(strstr(line,"pattern") != NULL) flag = 0;
				else if(strstr(line,"complex") != NULL) flag = 2;
				else flag = 1;
			}	
		}
		infile.unget();
...
//fprintf(stderr, "%d %d %d\n", m, n, nnz);
		...
		if (infile.is_open())
		{
			...
			{
				if(flag == 1) infile >> triples[cnz].row >> triples[cnz].col >> triples[cnz].val;	// row-col-value
				else if(flag == 2) infile >> triples[cnz].row >> triples[cnz].col >> triples[cnz].val >> ttemp;	// row-col-value
				else if(flag == 0)  infile >> triples[cnz].row >> triples[cnz].col;
				triples[cnz].row--;
				triples[cnz].col--;
				++cnz;
			}
...
//fprintf(stderr, "%d %d %d %d\n", m, n, cnz, nnz);
	...
	double t0_ = timer_seconds_since_init();
	for(int i=0; i < ITER*4; ++i)
	{
		bicsb_gespmv<PTARR>(bicsb, &(x[0]), &(y_bicsb[0]));
	}
	double t1_ = timer_seconds_since_init();
	...
	double time_ = (t1_-t0_)/ITER;
	double time = (t1-t0)/ITER;
	cout<< "BiCSB" << " time: " << time << " seconds" <<endl;
	cout<< "BiCSB" << " mflop/sec: " << mflops  / time <<endl;
	fprintf(stdout,"TTAAGG,%s,%f\n", argv[1],(double)mflops / time_ / 1000);
	fprintf(stdout,"TTAAGG,%s,%f\n", argv[1],(double)mflops / time / 1000);
	fprintf(fpo,"%f,", (double)mflops / time / 1000);
